    ...
    def do_POST(self):
        ...
        try:
            ...
            try:
                ...
                filename = request_data.get('filename', 'sample_video.mp4')
                content_type = request_data.get('content_type', 'video/mp4')
            except:
            ...
            if content_type not in allowed_types:
                self.send_error(400, "Invalid file type. Only video files are allowed.")
                return
            
            # Validate file size (max 100MB)
            max_size = 100 * 1024 * 1024  # 100MB
            if len(file_data) > max_size:
                self.send_error(400, "File too large. Maximum size is 100MB.")
                return
